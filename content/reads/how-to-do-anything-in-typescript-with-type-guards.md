...
[](#the-kinds-of-type-guards-how-to-check-a-type)The kinds of type guards (how to check a type)
-----------------------------------------------------------------------------------------------
...
### [](#boolean-type-guard-truthiness)Boolean type guard (truthiness)
...
### [](#equality-type-guard)Equality type guard
...
A **discriminated union** is a [union type](https://camchenry.com/blog/typescript-union-type) with a field that allows to us to discriminate (or differentiate) between different types. Discriminated unions are essentially a more powerful version of the equality type guard, that can quickly tell us what case we are dealing with in a union type. In other words, when we check the value of a single field (like \`type\`), it automatically includes a number of other properties.
...
### [](#assertion-type-guard-or-assertion-function)Assertion type guard (or assertion function)
...
### [](#user-defined-custom-type-guard)User-defined (custom) type guard
...
### [](#examples-of-type-guard-functions)Examples of type guard functions
...
#### [](#check-if-a-value-is-defined-not-null-or-undefined)Check if a value is defined (not null or undefined)
...
#### [](#remove-all-values-null-or-undefined-values-from-array)Remove all values `null` or `undefined` values from array
...
#### [](#check-if-a-string-is-a-guid)Check if a string is a GUID
...
#### [](#check-if-a-value-is-a-valid-react-element-reactisvalidelement)Check if a value is a valid React element (`React.isValidElement`)
...
### [](#pros-and-cons-of-custom-type-guard-functions)Pros and cons of custom type guard functions
...
[](#where-can-a-type-guard-be-used)Where can a type guard be used?
------------------------------------------------------------------
